...
describe("CodexAppServerClient", () => {
  ...
  test("spawns Windows command shims through the shell", async () => {
    const child = createFakeChildProcess();
    const writes: Array<Record<string, unknown>> = [];
    let spawnedOptions: { shell?: boolean } | null = null;
    let inputBuffer = "";
    ...
    const client = new CodexAppServerClient({
      spawnImpl: (_command, _args, options) => {
        spawnedOptions = options as { shell?: boolean };
        return child as never;
      },
      resolveCommandImpl: async () => ({
        configuredCommand: "C:\\nvm4w\\nodejs\\codex",
    ...
    expect(writes[0]).toMatchObject({ method: "initialize" });
